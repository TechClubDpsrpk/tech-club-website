...
export default function AccountPage() {
  ...
  const [_showPasswords, _setShowPasswords] = useState({
    current: false,
    new: false,
    confirm: false,
  });
  const [_uploadingAvatar, _setUploadingAvatar] = useState(false);
  ...
  const _showMsg = (type: 'success' | 'error', text: string) => {
    _setMessage({ type, text });
    setTimeout(() => _setMessage(null), 3000);
  };

  const _handleSaveProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name.trim()) return _showMsg('error', 'Name cannot be empty');
    ...
    try {
      ...
      } else _showMsg('error', 'Failed to update profile');
    } catch {
      _showMsg('error', 'An error occurred');
    } finally {
      _setSaving(false);
    }
  };

  const _handleUpdatePassword = async (e: React.FormEvent) => {
    e.preventDefault();

    if (passwordData.new.length < 8)
      return _showMsg('error', 'New password must be at least 8 characters');
    if (passwordData.new !== passwordData.confirm)
      return _showMsg('error', 'Passwords do not match');
    ...
    try {
      ...
      if (response.ok) {
        ...
        _showMsg('success', 'Password updated successfully');
      } else _showMsg('error', data.error || 'Failed to update password');
    } catch {
      _showMsg('error', 'An error occurred');
    } finally {
      _setSaving(false);
  ...
  const _handleLogout = async () => {
    try {
      ...
      _showMsg('error', 'Failed to logout');
    }
  };
  ...
  const _handleDeleteAccount = async () => {
    ...
    try {
      ...
      else _showMsg('error', 'Failed to delete account');
    } catch {
      _showMsg('error', 'An error occurred');
    } finally {
      _setSaving(false);
  ...
  const _handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) return _showMsg('error', 'Please upload an image file');
    if (file.size > 5 * 1024 * 1024) return _showMsg('error', 'Image must be less than 5MB');
    ...
    try {
      ...
      if (response.ok) {
        ...
        _showMsg('success', 'Profile picture updated');
      } else _showMsg('error', 'Failed to upload image');
    } catch {
      _showMsg('error', 'An error occurred');
    } finally {
  ...
  const _handleResendVerification = async () => {
    ...
    try {
      ...
      if (response.ok) _showMsg('success', 'Verification email sent. Check your inbox!');
      else _showMsg('error', 'Failed to send verification email');
    } catch {
      _showMsg('error', 'An error occurred');
    } finally {
...
type _InputProps = { label: string } & React.InputHTMLAttributes<HTMLInputElement>;
...
function _Card({ title, icon: Icon, children, danger = false }: _CardProps) {
  return (
    <div
...
function _Input({ label, ...props }: _InputProps) {
  return (
...
function _PasswordInput({ label, show, onToggle, ...props }: _PasswordInputProps) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-medium text-gray-400">{label}</span>
...
function _PrimaryBtn({ children, ...props }: _ButtonProps) {
  return (
    <button
      {...props}
...
function _GhostBtn({ children, ...props }: _ButtonProps) {
  return (
    <button
      {...props}
...
function _DangerBtn({ children, ...props }: _ButtonProps) {
  return (
    <button
      {...props}
...
function _InfoRow({ label, value, mono = false }: _InfoRowProps) {
  return (
...
function _BadgeRow({ icon: Icon, label, tone }: _BadgeRowProps) {
  return (
    <div
...
function _SessionRow({ name, current = false }: _SessionRowProps) {
  return (
